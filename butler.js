...
let https                = require('https'),
   ...
   server              = https.createServer({
    ...
    pN                  = require('./components/butler-call/push-notifications')({config: config}),
    winston             = require('winston'),
    logPath             = path.join(__dirname, config.inpremise.server.logs.path.accesslog),
    logger              = require('./components/logger').log(),
    ...
    oneAuthAPI        = require('./components/one-auth/')({}),
...
app.post('/authenticate', function(req, res) {
  if("client_id" in req.body && "client_secret" in req.body) {
    oneAuthAPI.getToken(req.body, function(eo, ro){
      if(ro) {
...
app.post('/request-to-complete', function(req, res){
  ...
  {
    let resText = {};
    connection.query('UPDATE `butler_request_map` SET resolution_time=NOW(), remark="'+remark+'", source_of_complete="DASHBOARD" WHERE request_id="'+rid+'"', function(err, rows, fields) {
...
butlerApp._sendPNBulk = function (params) {
    if(params.length > 0)
    {
        ...
        connection.query('select bi.*, bst.*, bsm.*, bzm.*, zonemap.*, baad.butler_app_active_devices_id, baad.push_notifications, baad.device_id from butler_info as bi inner join butler_shift_map as bsm on bi.butler_info_id=bsm.butler_info_id inner join butler_shift_timing as bst on bst.shift_id=bsm.shift_id inner join butler_zones_master as bzm on bzm.butler_zones_master_id=bsm.zone_id inner join butler_zone_map as zonemap on zonemap.zone_id=bsm.zone_id inner join butler_app_active_devices as baad on bi.butler_app_users_id="'+butler_app_users_id+'" where bi.is_deleted=0 and bst.is_deleted=0 and bzm.is_deleted=0 and bi.butler_app_users_id="'+butler_app_users_id+'" and baad.device_id="'+req.body.device_id+'"', function(err, rows, fields) {
          ...
          if(rows.length > 0)
          {
            ...
            rows.forEach(function(item){
                ...
                if(tempShift.indexOf(item.shift_id) == -1) {
                  shiftArray.push({
                  ...
                  tempShift.push(item.shift_id);
                }

                if(tempRoom.indexOf(item.key_number) == -1) {
                  roomArray.push({
                      'key_number': item.key_number
                  });
                  tempRoom.push(item.key_number);
                }
            });
...
console.log('SELECT count(*) as adcount FROM `butler_app_active_devices` WHERE butler_app_users_id="'+data.butler_app_users_id+'" AND device_id="'+req.body.device_info.uniqueDeviceID+'"');
connection.query('SELECT count(*) as adcount FROM `butler_app_active_devices` WHERE butler_app_users_id="'+data.butler_app_users_id+'" AND device_id="'+req.body.device_info.uniqueDeviceID+'"', function(err, rows, fields) {
 ...
 if (err) { sendError(res, "Server error occurred. Please try after some time."); }
      ...
      {
	console.log('INSERT INTO `butler_app_active_devices`(butler_app_users_id, is_logged_in, last_login, device_token, device_info, device_id, device_type) VALUES("'+data.butler_app_users_id+'", 1, NOW(), "'+req.body.device_token+'", "'+encrypt_str(JSON.stringify(req.body.device_info))+'", "'+req.body.device_info.uniqueDeviceID+'", "'+req.body.device_type+'")');
...
var getAuthenticate = function(data, res, callback) {
    ...
    {
        console.log('SELECT bau.butler_app_users_id FROM `butler_app_users` as bau inner join butler_info as bi ON bau.butler_app_users_id=bi.butler_app_users_id WHERE bau.username="'+u+'" AND bau.password="'+p+'" AND bau.is_active=1 and bi.is_deleted=0');
        connection.query('SELECT bau.butler_app_users_id FROM `butler_app_users` as bau inner join butler_info as bi ON bau.butler_app_users_id=bi.butler_app_users_id WHERE bau.username="'+u+'" AND bau.password="'+p+'" AND bau.is_active=1 and bi.is_deleted=0', function(err, rows, fields) {
          if (err) { sendError(res, "Server error occurred. Please try after some time."); }
          console.log(rows);
          callback(null, rows);
        });
...
function closeRequestCall(roomno, remarks, requestId, callback)
{
    let headers = {
      ...
      'Access-Token': config.inpremise.accessToken
    };
            ...
            recentRequestProcessForMobileApps(data, function (error, recentRequestData){
                ...
                if(recentRequestData)
                {
                    ...
                    getUpdatedStatus(data, function(err, updatedData){
                        ...
                        butlerApp._sendPNBulk(updatedData);
                    });
                }
            });
        }, 5000);
...
butlerC.on('connection', function (socket) {
    socket.on('incomingRequest', function (data) {
        incomingRequestProcess(data, function(error, justArrivedRequests, t){
            ...
            if(justArrivedRequests.length)
            {
                ...
                butlerView += '<th width="180px">Guest Name</th>';
                butlerView += '<th width="220px">Butler Id</th>';
                butlerView += '<th width="240px">Butler Name</th>';
                butlerView += '<th width="340px" class="text-large">Time of Request</th>';
                ...
                getAlertMap(function(err, a){
                    ...
                    justArrivedRequests.forEach(function(obj){
                        ...
                        var escalationLevel = '';
                        if((obj.escalation_level).toLowerCase()=='checkin' ||
                        (obj.escalation_level).toLowerCase()=='checkout') {
                          escalationLevel = escalationLevel = (obj.escalation_level).toLowerCase();
                        } else if(parseInt(obj.escalation_level) > 0) {
                          escalationLevel = ' Level ' + obj.escalation_level;
                        } else {
                          escalationLevel = 'Just Arrived';
                        }

                        //escalationLevel = obj.escalation_level > 0 ? ' Level ' + obj.escalation_level : ' Just Arrived ';
                        ...
                        butlerView += '<td data-alert'+obj.request_id+'>' + escalationLevel + '</td>';
                        butlerView += '<td>';
                        if(escalationLevel=='checkin' || escalationLevel=='checkout') {
                          butlerView += '<button class="btn btn-green" ';
                          butlerView += 'data-closecall="'+obj.key_number+'"';
                          butlerView += 'data-requestid="'+obj.request_id+'"';
                          butlerView += '> Close Call </button>';
                        } else {
                          butlerView += '&nbsp;';
                        }
                        butlerView += '</td>';
                    ...
                    butlerView += '</tbody>';
                    butlerView += '</table>';
                    socket.emit('incomingRequest', { 'butlerView': butlerView, 'lastButlerReqId': lastButlerReqId, 'totalRecords': t.total, 'noRecent': data.noRecent, 'alertColors': temp });
                });
            }
            else
            {
            ...
            if(data.length)
            {
                ...
                getAlertMap(function(err, a){
                    ...
                    data.forEach(function(obj){
                        ...
                        if((obj.escalation_level).toLowerCase()=='checkin' ||
                        (obj.escalation_level).toLowerCase()=='checkout') {
                          completedAt = (obj.escalation_level).toUpperCase() + ' Completed';
                        } else if(parseInt(obj.escalation_level) > 0) {
                          completedAt = ' Completed at Level ' + obj.escalation_level + ' state';
                        } else {
                          completedAt = ' Completed at Just Arrived state';
                        }
            ...
            if(data.length)
            {
                ...
                getAlertMap(function(err, a){
                    ...
                    data.forEach(function(obj){
                        ...
                        var bgColor = '';
                        if(parseInt(obj.escalation_level) > 0) {
                          bgColor = 'background: ' + temp[obj.escalation_level];
                        }
                        //  data-requestBg="'+requestStatusColor+'"
                        butlerView += '<tr data-alertlevel-'+obj.escalation_level+'="1" style="'+bgColor+'"><td>' + obj.key_number + '</td>';
                        butlerView += '<td>' + obj.butler_id + ' ' + butlerDeleted + '</td>';
                        butlerView += '<td>' + obj.butler_name + '</td>';
                        butlerView += '<td>' + moment(obj.time_of_request).format('LLL') + '</td>';
                            ...
                            getAlertMap(function(err, a){
                                ...
                                recentRequestData.forEach(function(obj){
                                    ...
                                    (obj.escalation_level).toLowerCase()=='checkout') {
                                      ...
                                      parseInt(obj.escalation_level) > 0
                                    ) {
                                      escalationLevel = ' Level ' + obj.escalation_level;
                                    } else {
                                      escalationLevel = 'Just Arrived';
                                ...
                                socket.emit('recentRequest', { 'butlerView': butlerView, 'updatedData': updatedData, 'lastButlerReqId': lastButlerReqId, 'recentRequestRooms': recentRequestRooms, alertColors: temp });
                            });
                        }

                        if(updatedData.length > 0 && !recentRequestData.length)
                        {
                          getAlertMap(function(err, a){
                              var temp = [];
                              a.forEach(function(i){
                                ...
                                if(rows.length > 0)
                                {
                                    ...
                                    rows.forEach(function(item){
                                        ...
                                        if(shiftId!=item.shift_id)
                                        {
                                            ...
                                            zoneView    += '<th width="20%">Butler Email</th>';
                                            zoneView    += '<th width="10%">Butler SMS Number</th>';
                                            zoneView    += '<th width="20%">Action</th>';
                                            zoneView    += '</thead></tr>';
                                            zoneView    += '<tbody>';
                                        }
                                        serialNo++;
					console.log('item.butler_info_id = ' + item.butler_info_id);
                                        zoneView    += '<tr>';
    ...
    socket.on('getButlers', function(data){
        getButlerNames(function(error, butlers){ //getButlerZones
		    ...
		    console.log(item.butler_info_id);
                    butlersView    += '<tr>';
    ...
    socket.on('getEditShiftView', function(data){
        getEditShiftView(data, function(error, view){
            ...
            if(view.length)
            {
                ...
                editShiftView += '<input class="form-control" type="text" name="edit_shift_start_time" value="'+view.time_start+'" readonly>';
                editShiftView += '</div>';

                editShiftView += '<div class="col-md-12">';
                editShiftView += '<label>Shift End Time *</label>';
                editShiftView += '<input class="form-control" type="text" name="edit_shift_end_time"  value="'+view.time_end+'" readonly>';
                editShiftView += '</div>';
    ...
    socket.on('getSideFilters', function(data){
      console.log('getSideFilters called');
      getSideFilters(function(error, filterDataView){
          if(error) {}
          var filterDataView = filterDataView.filterViewData;
          if(filterDataView.length)
          {
            var filterView = '';
            filterDataView.forEach(function(filterheadingData){
              filterView += '<table class="table">';
              filterView += '<thead>';
              filterView += '<tr class="heading table-checkbox">';
              filterView += '<th colspan="4">';
              filterView += filterheadingData['title'];
              filterView += '</th>';
              filterView += '</tr>';
              filterView += '</thead>';
              filterView += '<tbody>';
              if(
              filterheadingData['type']=='checkbox' &&
              filterheadingData['type_val']==1) {
                var l         = 0;
                var idfield   = ((filterheadingData['title']).toLowerCase()).replace(" ", "_");
                var idfield1  = idfield;
                var count     = (filterheadingData['filterOptionArray']).length;

                for(j=0; Math.ceil(count/2) > j; j++) {
                    filterView += '<tr class="odd gradeX" id="odd_gradeX_' + idfield1 + '">';
                    for(k=0; 2 > k; k++) {
                      if(
                        typeof filterheadingData['filterOptionArray'][l]!=='undefined' &&
                        typeof filterheadingData['filterOptionArray'][l] !== 'undefined') {
                          filterView += '<td>';
                          filterView += `<input
                          type="checkbox"
                          name="rating"
                          class="checkboxes"
                          data-category="keywordFilter"
                          value="` + filterheadingData['filterOptionArray'][l]['value'] + `"
                          data-` + filterheadingData['filterOptionArray'][l]['status'] + `=` +
                          filterheadingData['filterOptionArray'][l]['value'] +
                          `/>`;
                          filterView += '</td>';
                          filterView += '<td>';
                          filterView += filterheadingData['filterOptionArray'][l]['title'];
                          filterView += '</td>';
                          l++;
                        } else {
                          filterView += '<td>&nbsp;</td>';
            ...
            socket.emit('getSideFilters', { 'filterView': filterView });
          }
        });
    });
    ...
    socket.on('getMobileAppView', function(data){
        ...
        getMobileAppView(data, function(err, r){
            ...
            mobileAppView += '<label class="col-md-6"><input type="radio" name="user_status" '+inactive+' value="0"> Inactive </label>';
            mobileAppView += '</div>';
            mobileAppView += '</div>';
...
{
    console.log('select username from butler_app_users where username="'+data.app_username+'" where is_deleted=0');
    connection.query('select bau.username from butler_app_users as bau inner join butler_info as bi on bi.butler_app_users_id=bau.butler_app_users_id where bau.username="'+data.app_username+'" and bi.is_deleted=0', function(err, rows, fields) {
        if (err) throw err;
        console.log(rows);
        if(rows.length > 0)
        {
...
function saveEditShift(data, callback)
{
    ...
    if(shiftid)
    {
        if(data.shift_end_time=='00:00' || data.shift_end_time=='00:00:00') {
          data['shift_end_time'] = '23:59:59';
        }
...
{
    console.log('FUNCTION addZoneRoomNumber');
    console.log(data);
    ...
    if(data.key_number)
    {
        ...
        console.log('INSERT INTO `butler_zone_map`(key_number, zone_id) VALUES ' + values);
...
function addZoneButlers(data, zone_id, callback)
{
    ...
    if(data.butlers)
    {
        ...
        if(values)
        {
            console.log('INSERT INTO `butler_shift_map`(zone_id, shift_id, butler_info_id) VALUES ' + values);
...
var incomingRequestProcess = function(data, callback)
{
   ...
   getTotalIncomingRequest(filter, function(err, t){
      connection.query("SELECT pg.guest_title, pg.guest_first_name, pg.guest_last_name, bi.*, zonemaster.zone_name, zonemaster.is_deleted as zone_is_deleted, bst.is_deleted as shift_is_deleted, brm.butler_request_map_id, brm.key_number, brm.request_id, brm.escalation_level, brm.time_of_request, brm.butler_info_id, brm.remark, bsm.shift_id, bst.shift_name FROM butler_request_map as brm INNER JOIN butler_shift_map as bsm on brm.butler_info_id = bsm.butler_info_id INNER JOIN butler_shift_timing as bst ON bst.shift_id=bsm.shift_id INNER JOIN butler_info as bi ON bi.butler_info_id=brm.butler_info_id INNER JOIN butler_zones_master as zonemaster ON zonemaster.butler_zones_master_id=brm.zone_id INNER JOIN pmsi_guests as pg on pg.pmsi_guest_id=brm.pmsi_guest_id WHERE brm.resolution_time IS NULL AND brm.shift_id=bst.shift_id AND brm.zone_id=bsm.zone_id AND bi.is_deleted=0 " + filter + " ORDER BY brm.butler_request_map_id DESC, brm.escalation_level DESC LIMIT " + data.limit, function(err, rows, fields) {
...
var recentRequestProcess = function(data, callback)
{
    if(typeof data.lastReqId!='undefined')
    {
        ...
        connection.query("SELECT pg.guest_title, pg.guest_first_name, pg.guest_last_name, bi.*, brm.butler_request_map_id, brm.key_number, brm.request_id, brm.escalation_level, brm.time_of_request, brm.butler_info_id, bsm.shift_id, bst.shift_name, bzm.zone_name FROM butler_request_map as brm INNER JOIN butler_shift_map as bsm on brm.butler_info_id = bsm.butler_info_id INNER JOIN butler_shift_timing as bst ON bst.shift_id=bsm.shift_id INNER JOIN butler_info as bi ON bi.butler_info_id=brm.butler_info_id INNER JOIN butler_zones_master as bzm ON bzm.butler_zones_master_id=brm.zone_id INNER JOIN pmsi_guests as pg ON pg.pmsi_guest_id=brm.pmsi_guest_id WHERE 1 AND brm.resolution_time IS NULL AND brm.butler_request_map_id > " + data.lastReqId + " AND brm.shift_id=bsm.shift_id AND brm.zone_id=bsm.zone_id AND brm.butler_info_id=bsm.butler_info_id ORDER BY brm.butler_request_map_id DESC LIMIT " + data.limit, function(err, rows, fields) {
            if (err) throw err;
            callback(null, rows);
        });
    }
...
function actionOnRoomNumber(mid, data, callback)
{
    ...
    roomIdList += ' ) ';
    connection.query('SELECT pg.roomno, pg.guestid, pg.guestname, pg.pc_ip, dv.room_no, dv.room_description FROM pmsi_guest as pg RIGHT JOIN digivalet_status as dv ON pg.pc_ip=dv.pc_ip INNER JOIN keys as r ON r.number=pg.roomno WHERE pg.changeflag=0 AND dv.room_description NOT LIKE "%MstController%" AND r.room_id IN ' + roomIdList +'  GROUP BY pg.pc_ip' , function(err, rows, fields) {
      if (err) throw err;
        callback(null, rows);
    });
}
...
{
    connection.query('SELECT DISTINCT(`number`) as key_number FROM `keys`', function(err, rows, fields) {
      if (err) throw err;
        var arr = [];
        if(rows.length > 0)
        {
...
function getRoomNumbers(zoneid, callback)
{
    ...
    getRegisteredRooms(zone_id, function(err, data){
        ...
        if(roomNumberString){ inClause = ' WHERE  number NOT IN ( ' + roomNumberString + ' ) '; }
        connection.query('SELECT DISTINCT(`number`) as key_number FROM `keys` ' + inClause + ' ORDER BY key_number ASC', function(err, rows, fields) {
          if (err) throw err;
            callback(null, rows);
        });
    });
...
{
    console.log(data.butlerid);
    console.log(decrypt_str(data.butlerid));
    var butlerid = decrypt_str(data.butlerid);
    if(butlerid)
    {
...
function getSideFilters(callback) {
  connection.query('SELECT * FROM `butler_filterheadings` as dfh WHERE dfh.status=1 and type="checkbox" ORDER BY dfh.sort_order',
    function(err, rows, fields) {
    if (err) throw err;
      if(rows.length > 0) {
        var filterViewData = [];
        var counter = 0;
        rows.forEach(function(iData){

          connection.query(
            'SELECT * FROM `butler_filteroptions` WHERE filter_heading_id="' + iData.filterheading_id + '"',
            function(err, filterOptionsData, fields) {
              ...
              filterOptionsData.forEach(function(filterOptionData) {
                filterOptionArray.push({
                 'filteroption_id': filterOptionData.filteroption_id,
                 'filter_heading_id': filterOptionData.filter_heading_id,
                 'title': filterOptionData.title,
                 'value': filterOptionData.value,
                 'status': filterOptionData.status,
                });
              });

              filterViewData.push({
                'filterheading_id': iData.filterheading_id,
                'title': iData.title,
                'status': iData.status,
                'sort_order': iData.sort_order,
                'type': iData.type,
                'type_val': iData.type_val,
                'filterOptionArray': filterOptionArray
              });
              counter++;
              if(counter==rows.length) {
                callback(null, { 'filterViewData': filterViewData });
              }
          });
        });

      } else {
        callback('No butler filtres under this category', false);
      }
  });
}
